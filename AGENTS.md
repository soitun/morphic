...
- Run a single test file: `bun run test path/to/test.test.ts`
- Run tests matching a pattern: `bun run test --grep "test name"`
...
- Use **Tailwind CSS** for styling (via tailwind-merge + clsx pattern)
- Use shadcn/ui components from `/components/ui/` when available

### Imports

Imports are auto-sorted by ESLint using `eslint-plugin-simple-import-sort`. Order:

1. React and Next.js (`^react`, `^next`)
2. Third-party packages (`^@?\w`)
3. Internal imports (`@/types`, `@/config`, `@/lib`, `@/hooks`, `@/components/ui`, `@/components`, `@/registry`, `@/styles`, `@/app`)
4. Side effects (`^\u0000`)
5. Parent/relative imports (`^\\.\\.`, `^\\./`)
6. CSS imports (`.css`)

Run `bun lint --fix` to auto-fix import order.

### Naming Conventions

- **Components**: PascalCase (`ChatInput.tsx`, `SearchResults.tsx`)
- **Hooks**: camelCase with `use` prefix (`useChat.ts`, `useSearch.ts`)
- **Utilities**: camelCase (`formatDate.ts`, `cn.ts`)
- **Types/Interfaces**: PascalCase (`UserProfile.ts`, `SearchResult.ts`)
- **Files**: kebab-case for non-component files (`chat-db.ts`, `api-handler.ts`)

### Types

- Prefer explicit types over inference for function parameters and return values
- Use interfaces for object shapes, types for unions/intersections
- Use Zod schemas in `/lib/schema/` for runtime validation
- Avoid `any`; use `unknown` when type is truly unknown

### Error Handling

- Use try/catch with async functions; return typed error responses
...
- Place components in `/components/` with feature-based subdirectories
- Co-locate tests next to source files: `Component.tsx` and `Component.test.tsx`
- Use `use client` directive for client-side components
- Extract reusable UI to shadcn/ui in `/components/ui/`

### Database & API

- Use Drizzle ORM for database operations
- Place database schema in `/lib/db/schema.ts`
